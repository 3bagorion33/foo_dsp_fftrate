...
foo_dsp_fftrate_dialog::foo_dsp_fftrate_dialog( foo_dsp_fftrate_params &p_params ):
	m_params( p_params ), m_rateList(p_params.get_rateList())
{
}
...
BOOL foo_dsp_fftrate_dialog::OnInitDialog( CWindow wndFocus, LPARAM lInitParam )
{
	...
	uSetDlgItemText(*this, IDC_EDIT_RATES, pfc::stringcvt::string_ansi_from_utf8(m_rateList));
...
void foo_dsp_fftrate_dialog::OnCommand(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	case IDOK:
		...
		m_rateList = uGetDlgItemText(*this, IDC_EDIT_RATES);
		m_params.set_rateList(m_rateList);
		EndDialog(IDOK);
		break;
