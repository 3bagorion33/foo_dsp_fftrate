	...
	m_map( 0 )
{
	...
	m_rateList = params.get_rateList();
}
...
std::vector<unsigned> split_string(const std::string& str, char delimiter) {
...
void foo_dsp_fftrate::open( int p_rate, int p_chann, int p_map )
{
	...
	std::vector<unsigned> rates_to_skip = split_string(m_rateList.c_str(), ';');
	std::set<unsigned> skip_rates(rates_to_skip.begin(), rates_to_skip.end());
	...
	while( 1 )
	{
		...
		if (skip_rates.find(format_src->nSamplesPerSec) != skip_rates.end())
			break;
