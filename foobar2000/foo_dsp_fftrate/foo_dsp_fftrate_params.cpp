...
bool foo_dsp_fftrate_params::set_data(const dsp_preset& p_data)
{
    ...
    const char* rateList_data = reinterpret_cast<const char*>(raw_data + dataLen);
    m_rateList = pfc::string8(rateList_data);
...
bool foo_dsp_fftrate_params::get_data(dsp_preset& p_data)
{
    ...
    std::vector<char> data((char*)temp, (char*)temp + m_paramCount * sizeof(t_int32));
    data.insert(data.end(), m_rateList.c_str(), m_rateList.c_str() + m_rateList.length() + 1);

    p_data.set_data(data.data(), data.size());
    return true;
}
